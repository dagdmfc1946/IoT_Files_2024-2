...
int ledMQTT = 5;        // LED indicador de conexión MQTT
...
void reconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect("esp32")) {
      ...
      digitalWrite(ledMQTT, HIGH); // Encender LED MQTT cuando esté conectado
    } else {
      Serial.print("Fallo, rc=");
      Serial.print(mqttClient.state());
      Serial.println(" intentar de nuevo en 5 segundos");
      digitalWrite(ledMQTT, LOW); // Apagar LED MQTT si la conexión falla
...
void setup() {
  ...
  pinMode(ledMQTT, OUTPUT);  // Configuración del LED para el estado de MQTT
...
void loop() {
  ...
  delay(1500); // Espera antes de la siguiente lectura
}
